...
public class ReceiveSmsMessageAction extends Action implements Parcelable {
    ...
    public ReceiveSmsMessageAction(final ContentValues messageValues) {
        //Parcelable接口，Bundle.putParcelable(key,Object),intent传递对象的一种方法，另一种方法是Serilizable接口，
        // Bundle.putSerilizable(key,Object)，Parcelable接口性能更高
    ...
    protected Object executeAction() {
        ...
        if (subId == null) {
            subId = ParticipantData.DEFAULT_SELF_SUB_ID;//-1
        }
        // Make sure we have a sender address
        String address = messageValues.getAsString(Sms.ADDRESS);//"address"
        //收到的sms没有地址则Smd.ADDRESS值为UNDOWN_SENDER
        ...
        //返回实现Parcelable接口的ParticipantData类的对象
        final ParticipantData rawSender = ParticipantData.getFromRawPhoneBySimLocale(
                address, subId);

        // TODO: Should use local timestamp（时间戳） for this?
        final long received = messageValues.getAsLong(Sms.DATE);//"data"
        // Inform(通知) sync that message has been added at local received timestamp
        ...
        messageValues.put(Sms.THREAD_ID, threadId);//"thread_id"
