...
public final class SmsReceiver extends BroadcastReceiver {
    ...
    public static void postNewMessageSecondaryUserNotification() {
        final Context context = Factory.get().getApplicationContext();//返回应用的上下文
        final Resources resources = context.getResources();//获得resources对象
        final PendingIntent pendingIntent = UIIntents.get()
                .getPendingIntentForSecondaryUserNewMessageNotification(context);//pendingintent，
                // 延迟的intent
        //NotificationCompat.Builder 实例化承载通知内容的对象
        final NotificationCompat.Builder builder = new NotificationCompat.Builder(context);
        //设置通知标题
        builder.setContentTitle(resources.getString(R.string.secondary_user_new_message_title))
                .setTicker(resources.getString(R.string.secondary_user_new_message_ticker))//设置初次收到通知时状态栏的文本内容
                .setSmallIcon(R.drawable.ic_sms_light)//使用小图标
        // Returning PRIORITY_HIH causes L to put up a HUD notification. Without it, the ticker
        // isn't displayed.
                .setPriority(Notification.PRIORITY_HIGH)//设置通知优先级
                .setContentIntent(pendingIntent);//通知被点击时发送的pendingIntent

        final NotificationCompat.BigTextStyle bigTextStyle =
                new NotificationCompat.BigTextStyle(builder);//设置通知样式为大型文本样式
        //Provide the longer text to be displayed in the big form of the template in place of the
        // content text
        ...
        //从context获得NotificationManagerCompat实例
        ...
        if (BugleNotifications.shouldVibrate(new SecondaryUserNotificationState())) {
            defaults |= Notification.DEFAULT_VIBRATE;//|=?使用默认手机震动
        }
    ...
    public static android.telephony.SmsMessage[] getMessagesFromIntent(Intent intent) {
        //广播接收短信意图，Read the PDUs out of an SMS_RECEIVED_ACTION
        ...
        try {
            //Returns the message body, or email message body if this message was from an email
            // gateway. Returns null if message body unavailable.
