        ...
        addButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (num.equals("") || seating.equals("")) {// 查看用户是否输入了台号和座位数
                    JOptionPane.showMessageDialog(null, "请输入台号和座位数！", "友情提示", JOptionPane.INFORMATION_MESSAGE);
                    return;
                }
                if (num.length() > 5) {// 查看台号的长度是否超过了5位
                    JOptionPane.showMessageDialog(null, "台号最多只能为5个字符！", "友情提示", JOptionPane.INFORMATION_MESSAGE);
                ...
                if (dao.sDeskByNum(num) != null) {// 查看该台号是否已经存在
                    JOptionPane.showMessageDialog(null, "该台号已经存在！", "友情提示", JOptionPane.INFORMATION_MESSAGE);
                    numTextField.requestFocus();// 为台号文本框请求获得焦点
                    return;
                }
        ...
        delButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRow == -1) {// 未选中任何餐台
                    JOptionPane.showMessageDialog(null, "请选择要删除的餐台！", "友情提示", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    ...
                    for (int row = 0; row < openedDeskTable.getRowCount(); row++) {// 查看该餐台是否正在被使用
                        if (deskNum.equals(openedDeskTable.getValueAt(row, 1))) {
                            JOptionPane.showMessageDialog(null, "该餐台正在使用，不能删除！", "友情提示", JOptionPane.INFORMATION_MESSAGE);
                            return;// 该餐台正在被使用，不能删除，返回
                        }
                    }
                    String infos[] = new String[] {// 组织确认信息
                    "确定要删除餐台：", "    台  号：" + deskNum, "    座位数：" + table.getValueAt(selectedRow, 2).toString() };
                    int i = JOptionPane.showConfirmDialog(null, infos, "友情提示", JOptionPane.YES_NO_OPTION);// 弹出确认提示
                    if (i == 0) {// 确认删除
