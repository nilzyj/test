...
public class ExclusiveOrOperationFrame extends JFrame {
    ExclusiveOrOperationPanel exclusiveOrOperationPanel = new ExclusiveOrOperationPanel(); // 创建面板类的实例
    public static void main(String args[]) { // 主方法
        ExclusiveOrOperationFrame frame = new ExclusiveOrOperationFrame(); // 创建窗体类的实例
        frame.setVisible(true); // 显示窗体
    }
    
    public ExclusiveOrOperationFrame() {
        super(); // 调用超类的构造方法
        setTitle("图形的异或运算"); // 窗体标题
        setBounds(100, 100, 395, 240); // 窗体的显示位置和大小
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 窗体关闭方式
        add(exclusiveOrOperationPanel); // 将面板类的实例添加到窗体容器中
    }
    
    class ExclusiveOrOperationPanel extends JPanel { // 创建内部面板类
